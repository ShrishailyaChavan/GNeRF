...
class PoseParameters(nn.Module):
    ...
    def init_poses_embed(self):
        if self.pose_mode == '3d':
            poses_embed = torch.tensor([[0., 0, 1]]).repeat(self.length, 1)  
        elif self.pose_mode == '6d':
            t = torch.tensor([[0., 0, 1]]).repeat(self.length, 1)  
            R = look_at_rotation(t, up=self.up)  
    ...
    def poses(self):
        if self.pose_mode == '3d':
            t = self.poses_embed[:, :3]  
            R = look_at_rotation(t, device=t.device)  
        elif self.pose_mode == '6d':
            t = self.poses_embed[:, :3]  
